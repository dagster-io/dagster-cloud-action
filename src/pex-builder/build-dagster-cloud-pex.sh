...
if [ -z "$DAGSTER_BRANCH" ]
then
    export DAGSTER_SOURCE_PACKAGE="dagster"
else
    export DAGSTER_SOURCE_PACKAGE="git+https://github.com/dagster-io/dagster.git@${DAGSTER_BRANCH}#egg=dagster&subdirectory=python_modules/dagster"
fi

echo "Building dagster-cloud.pex from "
echo " - $DAGSTER_CLOUD_SOURCE_PACKAGE"
echo " - $DAGSTER_CLOUD_CLI_SOURCE_PACKAGE"
echo " - $DAGSTER_SOURCE_PACKAGE"
echo ""
...
pex $DAGSTER_CLOUD_SOURCE_PACKAGE $DAGSTER_CLOUD_CLI_SOURCE_PACKAGE $DAGSTER_SOURCE_PACKAGE PyGithub \
    -o dagster-cloud.pex \
