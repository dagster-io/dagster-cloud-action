...
def build_source_pex(project_root, output_dir):

    os.makedirs(output_dir, exist_ok=True)
    out_pex_path = os.path.join(output_dir, "source.pex")

    # determine the top level source packages in the project
    source_packages = setuptools.find_packages(project_root)
    source_packages = [
    ...
    with tempfile.TemporaryDirectory() as src_dir:
        for pkg in source_packages:
            source_path = os.path.join(project_root, pkg)
